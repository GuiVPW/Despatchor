...
import { COMMENT_CREATED, COMMENT_REMOVED, pubsub } from '../../constants/subscriptions'
	...
	Mutation: {
		...
		removeComment: async (_, { input: { commentId, authorId } }): Promise<boolean> => {
			try {
				...
				if (!comment) throw new ForbiddenError('Comentário não existe ou já foi removido')
